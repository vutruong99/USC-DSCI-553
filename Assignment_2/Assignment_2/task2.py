...
if __name__ == "__main__":
    ...
    ta_feng_rdd = sc.textFile(input_file_path)
    headers = ta_feng_rdd.first()
    ta_feng_rdd = ta_feng_rdd.filter(lambda x: x != headers)
    ta_feng_rdd = ta_feng_rdd.map(lambda values: values.split(","))

    transaction_dt = ta_feng_rdd.map(lambda x: ((str(x[0].replace('"','')) + "-" + str(int(x[1].replace('"', '')))), str(int(x[5].replace('"','')))))

    result = []
    for item in transaction_dt.collect():
        result.append([item[0], item[1]])

    with open('customer_product.csv', 'w', newline='', encoding='utf-8') as f:
        write = csv.writer(f)
        write.writerow(["DATE-CUSTOMER_ID", "PRODUCT_ID"])
        write.writerows(result)

    start_time = time.time()
    customer_product_path = "customer_product.csv"
    customer_product_rdd = sc.textFile(customer_product_path).map(lambda values: values.split(",")).filter(lambda x: x[0] != "DATE-CUSTOMER_ID")

    customer_product_rdd = customer_product_rdd.groupByKey().mapValues(lambda x: list(set(x))).filter(lambda x: len(x[1]) > filter_threshold).values()
    
    length = customer_product_rdd.getNumPartitions()

    candidates = customer_product_rdd.mapPartitions(lambda x: first_pass(x, length, support)).collect()

    frequent_itemsets = customer_product_rdd.mapPartitions(lambda x: second_pass(x, candidates)).reduceByKey(lambda x,y: x+y).\
